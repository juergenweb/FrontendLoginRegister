...
class RegisterPage extends FrontendLoginRegisterPages
{
    ...
    public function __construct(string $id = 'register-form')
    {
        ...
        // Show or hide the display of the toggle checkbox next to password fields. Unlike the
        // profile form, the password field is mandatory for the registration form (enforced by
        // FrontendLoginRegister::validateFormFields(), which blocks saving the module config
        // without it), so the elements are guaranteed to exist here.
    ...
    {
        $user = $this->wire('users')->get('email=' . $this->wire('sanitizer')->selectorValue($email));
     ...
     * @param bool $timestamp - true: return a Unix timestamp instead of a formatted date/time string
     * @return string|int - a "Y-m-d H:i" formatted string, or a Unix timestamp when $timestamp is true
     * @throws WireException
     * @throws WirePermissionException
     */
    protected function getDeleteDateTime(bool $timestamp = false): string|int
    {
        ...
        if ($this->isValid()) {
            ...
            if ($newUser->save()) {
                ...
                if ($this->wire('modules')->isInstalled('LanguageSupport')) {
                    ...
                    // Change the language on $this->user, NOT on $newUser. This looks backwards, but
                    // $this->user is the actual current-visitor User object (see
                    // Form::initUserAndLanguage(): $this->user = $this->wire('user')), and
                    // ProcessWire's _() translation calls further down (subject, title) resolve
                    // against wire('user')->language - not against an arbitrary other User object
                    // like $newUser.
                ...
                $activation_mail_sent = $this->sendLocalizedMail(
                    $newUser,
                    'input_activationtext',
                    [
                        'currentdatetimevalue' => $this->wire('datetime')->date($this->getDateFormat($newUser), time()),
                        'verificationlink' => $this->createActivationLink($newUser),
                        'notregisteredlink' => $this->createNotRegisteredLink($newUser),
                    ],
                    $this->_('Action required to activate your account'),
                    $this->_('Please click the link to verify your registration')
                );
                ...
                if ($email) {
                    ...
                    if (!is_null($user)) {
                        ...
                        if ($this->sendReminderMail($user)) {
                            ...
                            $this->setAlert('alert_warningClass', $this->_('It seems that you have already registered before but have forgotten to activate your account. To activate your account, please follow the instructions inside the mail that have been sent to you.'));
